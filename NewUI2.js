...
//CORE:
    createSetting('ManualGather2', ['收集/建造关闭', '自动收集/建造', '科学研究关闭', '自动收集/建造 #2'], '3路按钮。 食品，木材，金属（W / turkimp）与科学的自动采集。 自动变速构建您的构建队列。 现在可以将科学研究转化为实现“达”Z120而不用手工研究。 请测试“自动收集/建造 #2”，它还在实验阶段，但应该可以工作。', 'multitoggle', 1, null, "Core");
    createSetting('BetterAutoFight', ['更好的自动战斗关闭', '更好的自动战斗 1', '更好的自动战斗 2'], '3路按钮, 推荐的。 会自动处理战斗。 ＃2是新的，＃1是旧的算法（如果你有任何问题）。 新的BAF＃2确实如此：3）如果我们死亡，并且由于维度生成器而陷入循环中，我们可以随时添加时间（RemainingTime + ArmyAdd.Time＆GeneTimer）和4）点击 无论如何，如果我们死了，并且已经有＆gt; = 31 NextGroupTimer，然后再处理基因组学家的后果。 警告：如果您禁用BetterAutoFight进行自动运行，则直到您点击FIGHT，该游戏才会停止任何操作。 （不喜欢afk）', 'multitoggle', 1, null, "Core");
    createSetting('AutoStance', ['自动立场关闭', '自动立场 1', '自动立场 2'], '自动交换立场，以避免死亡。 请测试自动测试＃2，它非常实验，并在测试版。', 'multitoggle', 1, null, "Core");
    createSetting('BuyStorage', '购买存储', '当资源几乎满时，会购买存储。 （如自动存储，甚至预计Jestimp）', 'boolean', true, null, "Core");
    createSetting('BuyBuildings', '购买建筑', 'Will buy non storage buildings as soon as they are available', 'boolean', true, null, "Core");
    createSetting('BuyUpgrades', '购买升级', 'Autobuy non equipment Upgrades', 'boolean', true, null, "Core");
    createSetting('BuyJobs', '购买工作', 'Buys jobs based on ratios configured below. CAUTION: you cannot manually assign jobs with this. Toggle if you need to.', 'boolean', true, null, "Core");

    createSetting('TrapTrimps', '脆皮陷阱', 'Automatically trap trimps when needed, including building traps. (when you turn off, make sure you also turn off the ingame AutoTraps button)', 'boolean', true, null, "Core");

    createSetting('AutoHeirlooms', '自动传家宝', '自动评估和携带最好的传家宝，并推荐装备物品的升级。 当传家宝窗口未打开时，AutoHeirlooms将仅更改携带物品。 携带物品将与已经携带的类型进行比较和交换。 如果携带点是空的，它将被最好的盾牌填充（如果有的话）。 评估仅基于以下MOD（按优先级顺序排列，从高到低）：虚空地图掉落机会/ 脆皮攻击，暴击机会/暴击伤害，矿工效率/金属滴，宝石掉落/拖拉效率，农民/伐木工人的效率。 为了携带，罕见的胜过所有的统计评估。 空的mod插槽的值是最好的MOD的平均值。', 'boolean', true, null, "Core");
